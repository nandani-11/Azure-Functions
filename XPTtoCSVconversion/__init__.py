...
def main(myblob: azure.functions.InputStream):
    try:
        ...
        # df = pd.read_sas(myblob, format='xport')
        ...
        try:
            ...
            logging.info(f"Container '{output_container_name}' created.")
        except Exception as e:
            # If the container already exists, catch the exception and ignore it
            if not e.error_code == 'ContainerAlreadyExists':
                raise

        # # Get the container client to access the output container
        # container_client = blob_service_client.get_container_client(output_container_name)

        # Construct the full path for the output CSV file
        # sub_dir = "/".join(blob_dir.split("/")[1:])
        # output_blob_path = os.path.join(blob_dir, csv_filename)
